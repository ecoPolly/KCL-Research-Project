...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def _connect_signals(self):
        ...
        # self.clean_w.clicked.connect(lambda:self.compute_clean_work(show_plot=True))
    ...
    def plot_average_trajectory(self):
        ...
        self.grafico.plot(self.x_avg_cache, self.f_avg_cache, 
                         '-', lw=1.5, alpha=0.8, color='orange', label='Average Trace')
        self.grafico.legend(fontsize=6)
        self.grafico.tick_params(axis='both', which='major', labelsize=6)
        self.grafico.grid(True, alpha=0.3)
        self.grafico.figure.canvas.draw()
    ...
    def _plot_work_histogram(self):
        ...
        self.grafico_hist.set_xlabel('Total Work [pN·nm]', size=6.5)
        self.grafico_hist.set_ylabel('Probablity Density', size=6.5)
    ...
    def fit_wlc_global(self):
        ...
        if result.success:
            ...
            self.Label_WLCfit_dx.setText(f"Lc ={Lc_dx:.2f}nm   Lp={Lp_dx:.2f}nm")
            self.Label_WLCfit_sx.setText(f"Lc ={Lc_sx:.2f}nm   Lp={Lp_sx:.2f}nm   L0={L0:.2f}nm")
            
            self.grafico.clear()

            self.grafico.plot(self.x_avg_cache, self.f_avg_cache, 
                              '-', lw=1.2, color='orange', alpha=0.4, label='Average Trace')
            ...
            self.grafico.legend(fontsize=6) # poi rimuobi ashtag
    ...
    def compute_w_protein(self, show_plot=False):
        ...
        xmin = fsolve(lambda x: wlc_force(x, Lc, Lp, 0) - 0.01, 0.5 * Lc)[0]
        xmax = fsolve(lambda x: wlc_force(x, Lc, Lp, 0) - fmax, 0.5 * Lc)[0]
        Ws_p, _ = quad(lambda x: wlc_force(x, Lc, Lp, 0), xmin, xmax)

        if show_plot:
            fig, ax = plt.subplots(figsize=(7, 5))
            fig.canvas.manager.set_window_title('Protein Elastic Work Analysis')
            x = np.linspace(0, xmax + 0.5, 1000)
            force = wlc_force(x, Lc, Lp, 0)

            x_fill = np.linspace(xmin, xmax, 500)
            f_fill = wlc_force(x_fill, Lc, Lp, 0)
            ax.plot(x, force, color='dodgerblue', lw=1.5, label='WLC Model (Protein)')
            ax.fill_between(x_fill, f_fill, color='dodgerblue', alpha=0.3, 
                            label=f'Elastic Work ($W_p$) = {Ws_p:.1f} pN·nm')

            ax.axhline(y=fmax, color='red', linestyle=':', alpha=0.5, label=f'$F_{{max}}$ = {fmax} pN')
            ax.axvline(x=xmax, color='gray', linestyle=':', alpha=0.5)

            ax.set_title("Protein Stretching Contribution", fontsize=12, fontweight='bold') 
            ax.set_xlabel("Extension (nm)", fontsize=7)
            ax.set_ylabel("Force (pN)", fontsize=7)
            ax.legend(loc='upper left', frameon=True, shadow=True, fontsize=7)
            ax.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.show()
        print(f"W_protein: {Ws_p:.2f} pN·nm")
        self.Label_protein.setText("W= {:.2f} pN·nm".format(Ws_p))

        return Ws_p

    def compute_w_handles(self, show_plot=False):
        """Calculate elastic work for handle stretching"""
        Lc_dx, Lp_dx, Lc_sx, Lp_sx, L0 = self.fit_wlc_global()
        Lc = Lc_sx - L0 - self.Lc_fjc.value()
        Lp = Lp_sx
        ...
        if show_plot:
            fig, ax = plt.subplots(figsize=(7, 5))
            fig.canvas.manager.set_window_title('Handles Elastic Work Analysis')
            x = np.linspace(L0 , xmax + 0.5, 1000)
            force = wlc_force(x, Lc, Lp, L0)

            x_area = np.linspace(xmin, xmax, 200)
            f_area = wlc_force(x_area, Lc, Lp, L0)
            ax.plot(x, force, color='darkorange', lw=2, label='WLC Model (Handles)')
            ax.fill_between(x_area, f_area, color='darkorange', alpha=0.3, 
                            label=f'Elastic Work ($W_h$) = {Ws_h:.2f} pN·nm')

            ax.axhline(y=fmin, color='gray', linestyle=':', alpha=0.4, label=f'$F_{{min}}$ = {fmin} pN')
            ax.axhline(y=fmax, color='red', linestyle=':', alpha=0.4, label=f'$F_{{max}}$ = {fmax} pN')

            ax.set_title('Handles Stretching Contribution', fontsize=9, fontweight='bold')
            ax.set_xlabel('Extension (nm)', fontsize=7)
            ax.set_ylabel('Force (pN)', fontsize=7)
            ax.set_xlim(L0, xmax + 5)
            ax.set_ylim(0, fmax + 5)
            ax.legend(loc='upper left', frameon=True, fontsize=7)
            ax.grid(True, alpha=0.2)
            plt.tight_layout()
            plt.show()
        self.Label_handles.setText("W= {:.2f} pN·nm".format(Ws_h))

        return Ws_h

    # ========== FJC MODEL ==========

    def compute_fjc_work(self, show_plot=True):
        ...
        # Calcolo modello FJC
        F = np.linspace(0.01, fmax + 5, 1000)
        beta = F * lk / KBT
        fjc_x = Lc * (1 / np.tanh(beta) - 1 / beta)

        # Calcolo Lavoro di Orientamento (W_orient) fino a f0
        mask1 = F <= f0
        F_orient = F[mask1]
        x_orient = fjc_x[mask1]
        W_orient = simpson(x_orient[-1] - x_orient, F_orient)

        mask2 = (F >= f0) & (F <= fmax)
        F_stretch = F[mask2]
        x_stretch = fjc_x[mask2]
        W_stretch = simpson(x_stretch[-1] - x_stretch, F_stretch)

        self.Label_fjc.setText(f"{W_orient:.2f} pN·nm")
        self.Label_fjc_2.setText(f"{W_stretch:.2f} pN·nm")
        print(f"W_orient: {W_orient:.2f}, W_stretch: {W_stretch:.2f}")

        if show_plot:

            plt.figure("Handles Orientation Energy Contribution", figsize=(7, 5))
            plt.clf() 
            plt.plot(F,fjc_x, color='white', lw=1.5, label="FJC Model")
            plt.fill_between(F_orient, x_orient, x_orient[-1],
                            color="green", alpha=0.4, label=f"W_orient: {W_orient:.2f}")
            plt.fill_between(F_stretch, x_stretch, x_stretch[-1], 
                            color="red", alpha=0.4, label=f"W_stretch: {W_stretch:.2f}")

            plt.axvline(x=f0, color='yellow', linestyle='--', alpha=0.6, label=f"F_min ({f0} pN)")
            plt.axvline(x=fmax, color='orange', linestyle='--', alpha=0.6, label=f"F_max ({fmax} pN)")
            plt.axhline(y=x_orient[-1], color='black', linestyle=':', alpha=0.4)
            plt.xlabel("Force (pN)")
            plt.ylabel("Extension (nm)")
            plt.title("Handles Orientation (FJC Model)")
    ...
    def compute_clean_work(self, show_plot=False):
        """Subtract elastic contributions from total work"""
        work_list = self.compute_total_work()
        Ws_h = self.compute_w_handles(show_plot=True)
        Ws_p = self.compute_w_protein(show_plot=True)
        ...
        if show_plot:
            plt.figure("Work Distribution", figsize=(10, 5))
            plt.clf() 
    ...
    def compute_free_energy(self):
        ...
        _, work_cleaned = self.compute_clean_work(show_plot=True)
    ...
    def save_results(self):
        ...
        work_cleaned, _ = self.compute_clean_work(show_plot=False)        
        df = pd.DataFrame({
        ...
        filename = "work_results.csv"
        df.to_csv("work_results.csv", index=False)
        QtWidgets.QMessageBox.information(self, "Salvataggio", f"File '{filename}' salvato con successo!")
