...
def launch_Deconvolution():
    #  app = QtWidgets.QApplication(sys.argv)
    #  window = MyApp()
    #  window.show()
    #  sys.exit(app.exec_())  
    deconv_window = QWidget()
    deconv_window.setWindowTitle("Deconvolution Analysis")  # Imposta il titolo della finestra
    deconv_window.setGeometry(100, 100, 400, 300)  # Imposta le dimensioni della finestra

    layout = QVBoxLayout()
    label = QLabel("Deconvolution Functionality")
    layout.addWidget(label)
    deconv_window.setLayout(layout)
    deconv_window.show()

    return deconv_window 